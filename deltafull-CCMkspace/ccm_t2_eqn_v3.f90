...
!!$  endwtime = MPI_WTIME()
  ...
  call mpi_barrier(mpi_comm_world,ierror)
  endwtime = MPI_WTIME()
  if ( iam == 0 ) write(6,*) 'Time for I9', endwtime - startwtime
  ...
  do channel   = my_ph_hphp_channel_low(iam), my_ph_hphp_channel_high(iam) 
     ...
     dim1 = size( vnn_ph_hpph(channel3)%val,1 )
     dim2 = bra_max-bra_min + 1 !size( t2_ph_ccm(channel1)%val, 2)
     dim3 = size( vnn_ph_hpph(channel3)%val,2 )
  ...
  call mpi_barrier(mpi_comm_world,ierror)
  endwtime = MPI_WTIME()
  if ( iam == 0 ) write(6,*) 'Time for I4', endwtime - startwtime
  startwtime = MPI_WTIME()
