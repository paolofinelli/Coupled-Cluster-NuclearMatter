...
PROGRAM ccm_kspace 
  ...
  real*8, allocatable :: engles(:,:)
  complex*16 :: e0_av, mbpt2_av, eccsd_av, eccsdt_av,ener1
  integer :: nx, ny, nz, i , nxx, nyy, nzz, run
  integer :: spec_points(10,10,10),num_angles, angle_batch
...
!!$  stop
  ...
  case('TABCsp')
     if (.true.) then !finding special points
        if(iam==0) write(6,*) "calculating special point for dens = ",dens
        e0_av =0.d0 
        diff = 100.d0
        einf = -6.4019
        num_angles = 0
        ...
        if ( iam==0) write(6,*) "# of Twist angle combinations", num_angles
        
        !populate the twistangles combinations
        allocate(engles(num_angles,3))
        num_angles=0
        do nx = 1, size(twist_angle, 2)
           do ny = nx, size(twist_angle, 2)
              do nz = ny, size(twist_angle, 2)
                 num_angles = num_angles + 1
                 engles(num_angles,1) = nx
                 engles(num_angles,2) = ny
                 engles(num_angles,3) = nz
        ...
        call precalc_chp_functions
        !! number of angles calculated by each run
        angle_batch = int(num_angles/10)
        
        do run = angle_batch*my_twists+1, angle_batch*(my_twists+1)
!                 startwtime = MPI_WTIME()
           if(iam ==0) write(6,*) "Working on angle set:",run
           ...
           e0_av = factor * wxx(nx)*wxx(ny)*wxx(nz)*e0/(1.d0*pi)**3
           
           if(abs(e0_av/below_ef - einf) < diff) then
              diff = abs(e0_av/below_ef - einf)
              nxx = nx          
              nyy = ny          
              nzz = nz          
              e00 = e0_av/below_ef 
 ...
 !                if(iam == 0) write(6,*) "time: ", MPI_WTIME() - startwtime
           num_angles = num_angles + 1
        end do
        if(iam == 0) write(6,*) "special point (nx,ny,nz):",nxx,nyy,nzz,"e0:",e00
  ...
  case('TABC') 
     ...
     nx = engles(my_twists,1)
     ny = engles(my_twists,2)
     nz = engles(my_twists,3)
     if(iam ==0) write(6,*) "my twists are: ",nx,ny,nz
...
!!$     eccsdt_av = eccsdt_av + factor * wxx(nx)*wxx(ny)*wxx(nz)*eccsdt/(1.d0*pi)**3
!!$     if(iam ==0) write(6,*) "finished ccd calcs for my twist"
     !e0_av = e0 
!!$           mbpt2_av = mbpt2
!!$           eccsd_av = eccsd
     einf = -6.4019
...
!10 continue  
  ...
  if ( iam == 0 ) write(6,'(8f12.5)') cE, cD, real(kf), real(dens), real(e0_av/below_ef), &
  real(mbpt2_av/below_ef), real(eccsd_av/below_ef), real(eccsdt_av/below_ef)
...
!!$  end do
  ...
  call mpi_allreduce(hf_sum,hf_ene,1,mpi_complex16,mpi_sum, &
       mpi_comm_world,ierror)
  
  hf_ene = hf_ene/(2.*pi)**6/dens
  !if ( iam == 0 ) write(6,*) 'neutron matter', dens, real(hf_ene), real(hf_ene + 0.3d0* (2.d0*hbarc**2*kf**2/p_mass))
  if ( iam == 0 ) write(6,*) 'sym. nucl. matter NN contribution', dens, real(hf_ene)!, real(hf_ene + 0.3d0* (hbarc**2*kf**2/p_mass))
...
!!$           end if
  ...
  call mpi_allreduce(hf_sum,hf_3nf,1,mpi_complex16,mpi_sum, mpi_comm_world,ierror)
  
  if ( iam == 0 ) write(6,*) 'finished 3NF contribution:'  
  if(iam == 0 ) write(6,*) 'HF energy with NN: ',hf_ene
  hf_ene = hf_ene + hf_3nf/(2.d0*pi)**9/dens
  
  !if ( iam == 0 ) write(6,*) 'neutron matter', dens, real(hf_3nf), real(hf_ene + 0.3d0* (2.d0*hbarc**2*kf**2/p_mass))
  if ( iam == 0 ) write(6,*) 'sym. nucl. matter', dens, 'hf_3nf:',real(hf_3nf/(2.d0*pi)**9/dens), &
  'hf_energy:',hf_ene,'+Kinetic:',real(hf_ene + 0.3d0* (hbarc**2*kf**2/p_mass))
...
subroutine deallocate_structures
  ...
  if(iam == 0) write(6,*) 'deallocating channels'
  ...
  if(cc_approx .ne. 'mbpt2') deallocate( vnn_pppp )
  if(iam == 0) write(6,*) 'deallocating lookups'
  ...
  if(cc_approx .ne. 'mbpt2') then
     if(iam == 0) write(6,*) 'deallocating ph lookups'
  ...
  if(cc_approx .ne. 'mbpt2') deallocate(  lookup_pppp_configs ) 
  deallocate( locate_t2channel ) 
  deallocate( locate_channel ) 
  
  if(cc_approx .ne. 'mbpt2') then
     if(iam == 0) write(6,*) 'deallocating ph kets'
...
!  deallocate( pp_hppp%ival ) 
  if(iam == 0) write(6,*) 'deallocating channels'
  if(cc_approx .ne. 'mbpt2') deallocate( pp_pppp%ival ) 
  ...
  if(cc_approx .ne. 'mbpt2') DEALLOCATE( channels%pppp_quantum_numbers ) 
  
  if(cc_approx .ne. 'mbpt2') DEALLOCATE( channels%ph_phhp_quantum_numbers )
  if(cc_approx .ne. 'mbpt2') DEALLOCATE( channels%ph_hphp_quantum_numbers )
  if(cc_approx .ne. 'mbpt2') DEALLOCATE( channels%ph_hpph_quantum_numbers )
...
!!$  deallocate( vnn_ph_hpph )
  if(iam == 0) write(6,*) 'deallocating t2'
  if(cc_approx .ne. 'mbpt2') then
     do i = 1, size( t2_ccm )
  ...
  if(iam == 0) write(6,*) 'deallocating mpistuff'
  
  if(allocated(mapping)) deallocate( mapping ) 
  if(allocated( check_my_channel ) ) deallocate( check_my_channel ) 
  if(allocated(my_channel_low)) deallocate( my_channel_low)
  if(allocated( my_channel_high )) deallocate( my_channel_high )
  
  if(allocated(fully_stored_channel)) deallocate( fully_stored_channel )
  if(allocated( mapping_hhpp)) deallocate( mapping_hhpp ) 
  if(allocated( check_my_channel_hhpp )) deallocate( check_my_channel_hhpp ) 
  if(allocated( my_hhpp_channel_low) ) deallocate( my_hhpp_channel_low )
  if(allocated(my_hhpp_channel_high )) deallocate( my_hhpp_channel_high)
  if(allocated( mapping_hphp )) deallocate( mapping_hphp ) 
  if(allocated( check_my_channel_hphp )) deallocate( check_my_channel_hphp ) 
  if(iam == 0) write(6,*) 'deallocating ph mpistuff'
  if(cc_approx .ne. 'mbpt2') deallocate( mapping_ph_hphp )
  if(cc_approx .ne. 'mbpt2') deallocate( check_my_channel_ph_hphp )
  if(cc_approx .ne. 'mbpt2') deallocate( my_ph_hphp_channel_low, my_ph_hphp_channel_high) 
  ...
  if(iam == 0) write(6,*) 'deallocating done'
...
SUBROUTINE allocate_sp_data
  ...
  read(5,*);read(5,*) input, ntwist, nmax, my_twists
...
!!$  stop
  ...
  if ( iam == 0) write(6,*) 'Compute 3nf vacuum expectation value'
  !
  ...
  startwtime = MPI_WTIME()
  if(iam==0) write(6,*) 'Beginning openMP loop'
  !do i = 1, num_procs
  !   if ( iam == 0 ) write(6,*) 'iam',i,'lower/upper', nconf_low(i), nconf_high(i)
  !end do
  e0_3nf = 0.d0 
