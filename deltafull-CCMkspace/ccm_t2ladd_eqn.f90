  ...
  do channel   = 1, channels%number_hhpp_confs 
     ...
     do bra = bra_min, bra_max !1, size(  lookup_hhpp_configs(2,channel)%ival, 2) 
        ...
        do ket = 1, size(  lookup_hhpp_configs(1,channel)%ival, 2) 
           ...
           !t2_ccm_eqn(channel)%val(bra,ket) = chiral_pot(a,b,i,j)
     ...
     do bra = bra_min, bra_max !1, size(  lookup_hhpp_configs(2,channel)%ival, 2) 
        ...
        do ket = 1, size(  lookup_hhpp_configs(1,channel)%ival, 2) 
           ...
           do szc = -1, 1 ,2 
              ...
              if ( bra2 == 0 ) cycle 
              if ( b == c ) cycle 
              sum1 = sum1 + fock_mtx(b,c)*t2_ccm(channel)%val(bra2,ket)
           end do
           ...
           do szk = -1, 1, 2
              ...
              if ( ket2 == 0 ) cycle
              if ( k == j ) cycle 
              sum1 = sum1 - fock_mtx(k,j)*t2_ccm(channel)%val(bra,ket2)
           end do
     ...
     do bra = bra_min, bra_max !1, size(  lookup_hhpp_configs(2,channel)%ival, 2) 
        ...
        do ket = 1, size(  lookup_hhpp_configs(1,channel)%ival, 2) 
           ...
           do ket2 = 1, size(  lookup_hhpp_configs(1,channel)%ival, 2) 
              ...
              !sum2 = sum2 + 0.5d0* chiral_pot(k,l,i,j)*t2_ccm(channel)%val(bra,ket2)
              sum2 = sum2 + 0.5d0* vnn_hhhh(channel4)%val(bra3,ket3)*t2_ccm(channel)%val(bra,ket2)
           end do
...
SUBROUTINE setup_I6I7_intermediate
  ...
  I7 = fock_mtx(1:below_ef, 1:below_ef) 
  I6 = fock_mtx(below_ef+1:tot_orbs, below_ef+1:tot_orbs) 
