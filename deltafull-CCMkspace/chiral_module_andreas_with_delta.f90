module chiral_tables 
  ...
  real*8, allocatable,DIMENSION(:,:,:,:,:,:) :: R1_tab,R2_tab,R3_tab,R4_tab, & 
       R5_tab,R6_tab,r7_tab,r8_tab,r9_tab,r10_tab,r11_tab
...
CONTAINS 
  ...
  subroutine init_chp_constants 
    ...
    ! See Eq. 4.42 p. 26 
    cnlo(1) = (-5.d0*cnlo_pw(7)+6.d0*cnlo_pw(5)-3.d0*cnlo_pw(4)-3.d0*cnlo_pw(3)- & 
         cnlo_pw(2)+2.d0*cnlo_pw(1))/(64.d0*pi)
    cnlo(2) = ( 5.d0*cnlo_pw(7)+6.d0*cnlo_pw(5)+3.d0*cnlo_pw(4)+3.d0*cnlo_pw(3)+ &
         cnlo_pw(2)+2.d0*cnlo_pw(1))/(16.d0*pi)
    cnlo(3) = -( 2.d0*cnlo_pw(7)-2.d0*dsqrt(2.d0)*cnlo_pw(6)-2.d0*cnlo_pw(5)- & 
         3.d0*cnlo_pw(3)+cnlo_pw(2)+2.d0*cnlo_pw(1))/(64.d0*pi)
    cnlo(4) = -(-2.d0*cnlo_pw(7)-2.d0*dsqrt(2.d0)*cnlo_pw(6)-2.d0*cnlo_pw(5)+ & 
         3.d0*cnlo_pw(3)-cnlo_pw(2)+2.d0*cnlo_pw(1))/(16.d0*pi)
    ...
    N3LOST2PW = transpose(reshape((/ &
         1D0, 1D0/16D0, 0.25D0, 0D0, -3D0, -3D0/16D0, -0.75D0, 0D0, & 
         0D0, 0D0, -1D0, -0.25D0, -0.25D0, -1D0/16D0, 0D0, &
         10D0/3D0, 5D0/24D0, 1D0/6D0, 2D0/3D0, -10D0, -0.625D0, -0.5D0, & 
         -2D0, 0D0, 0D0, -10D0/3D0, -1D0/6D0, -1D0/6D0, -5D0/24D0, -2D0/3D0, &
         -4D0/3D0, 1D0/12D0, 0D0, 0D0, -4D0/3D0, 1D0/12D0, 0D0, 0D0, -2D0/3D0, & 
         -1D0/6D0, 8D0/3D0, 1D0/3D0, -1D0/3D0, -1D0/6D0, 0D0, &
         -4D0/3D0, 1D0/12D0, 0D0, 0D0, 4D0, -0.25D0, 0D0, 0D0, 0D0, 0D0, & 
         4D0/3D0, 0D0, 0D0, -1D0/12D0, 0D0, &
         -4D0/3D0, 1D0/12D0, 0D0, 0D0, -4D0/3D0, 1D0/12D0, 0D0, 0D0, -1D0/3D0, & 
         -1D0/12D0, -2D0, -1D0/6D0, 1D0/6D0, 1D0/8D0, 0D0, &
         1D0, 1D0/16D0, 0.25D0, 0D0, 1D0, 1D0/16D0, 0.25D0, 0D0, 0D0, 0D0, & 
         1D0/3D0, 1D0/12D0, 1D0/12D0, 1D0/48D0, 0D0, &
         10D0/3D0, 5D0/24D0, 1D0/6D0, 2D0/3D0, 10D0/3D0, 5D0/24D0, 1D0/6D0, & 
         2D0/3D0, 0D0, 0D0, 10D0/9D0, 1D0/18D0, 1D0/18D0, 5D0/72D0, 2D0/9D0, &
         8D0/15D0, 1D0/30D0, -2D0/15D0, -2D0/15D0, 8D0/15D0, 1D0/30D0, -2D0/15D0, & 
         -2D0/15D0, 2D0/5D0, -0.1D0, -4D0/9D0, 1D0/9D0, 1D0/9D0, -1D0/36D0, -16D0/45D0, &
         0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, -DSQRT(2D0)*2D0/3D0, & 
         -DSQRT(2D0)/6D0, -DSQRT(2D0)/6D0, -DSQRT(2D0)/24D0, 0D0, &
         0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, -DSQRT(2D0)*14D0/9D0, & 
         DSQRT(2D0)/18D0, DSQRT(2D0)/18D0, -DSQRT(2D0)*7D0/72D0, DSQRT(2D0)*2D0/9D0, &
         8D0/15D0, 1D0/30D0, -2D0/15D0, -2D0/15D0, -8D0/5D0, -0.1D0, 0.4D0, 0.4D0, &
         0D0, 0D0, -8D0/15D0, 2D0/15D0, 2D0/15D0, -1D0/30D0, 2D0/15D0, &
         8D0/15D0, 1D0/30D0, -2D0/15D0, -2D0/15D0, 8D0/15D0, 1D0/30D0, -2D0/15D0, & 
         -2D0/15D0, 2D0/15D0, -1D0/30D0, 0.8D0, -0.2D0, -0.2D0, 0.05D0, 4D0/15D0, &
         -4D0/3D0, 1D0/12D0, 0D0, 0D0, -4D0/3D0, 1D0/12D0, 0D0, 0D0, 1D0/3D0, & 
         1D0/12D0, -2D0/15D0, 1D0/30D0, -1D0/30D0, 1D0/120D0, 0D0, &
         0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, 0D0, DSQRT(6D0)*4D0/15D0, & 
         -DSQRT(6D0)/15D0, DSQRT(6D0)/15D0, -DSQRT(6D0)/60D0, 0D0, &
         8D0/15D0, 1D0/30D0, -2D0/15D0, -2D0/15D0, 8D0/15D0, 1D0/30D0, -2D0/15D0, & 
         -2D0/15D0, -4D0/15D0, 1D0/15D0, 0D0, 0D0, 0D0, 0D0, -2D0/15D0 &
         /), shape(N3LOST2PW)))
...
! Magic numbers, I got these from a Taylor expansion calculated in Mathematica
    ...
    REAL(8), PARAMETER, DIMENSION(0:appr_order) :: c = (/ 3593.92733718761d0, 5431.46429291511d0, & 
         3420.6675650193d0, 1149.1163208075d0, 217.17174525573d0, 21.89317093303d0, 0.9197592690185d0 /)
    REAL(8) :: L
    REAL(8) :: x, m_pi_inv
...
! Calculate the rest as a numerical integration from z_low to 0.95
      ...
      DO j = 1, z_nr
        ...
        z2 = z*z
        zt = sqrt(1 - z2)
        s = s + chp_2PE_2loop_int_mesh_z_VC_SFR%pnt(j)%w * & 
             (2-z2)*(z2-8)*(z2-2)*z*0.5d0*log((1+z)/(1-z)) / (a1 + z2 * q2)
      END DO
      s = s * q2**3
      x = 0.5*sqrt(x)
...
!    DO i = 1, nof_theta_int_points
      ...
      DO j = 1, z_nr
        ...
        z2 = z*z
        zt = sqrt(1 - z2)
        s = s + chp_2PE_2loop_int_mesh_z%pnt(j)%w * (1-z2) * & 
             ((z2-1)*z*0.5d0*log((1+z)/(1-z)) + (1+2*gA2)*z2) / (a1 + z2 * q2)
      END DO
      res = fact * q2**2 * s
 !   END DO
...
! impi  : determines which mpi2 to use,
  FUNCTION chp_delta_loop_H(q2, delta, w, sigma, L, impi) RESULT(res)
    ...
    IF (sfr_heavyside(impi) == 1.0D0) THEN
       ...
       delL = wdel * dlog( (sfr2*wdel*wdel+qdel2*sdel*sdel+2.0D0*sfr*qdel*wdel*sdel)/ & 
            ( fourmpi2(impi)*(sfr2+qdel2) ) )/(2.0D0*qdel)
...
!!$    print*,"delta_split: ", delta
  ...
  FUNCTION chp_NLO_delta_2PE_single_box_Wc_SFR(q2, L, S, D, delta, impi) RESULT(res)
    ...
    res = -gA2*hA2*const(22)*( (12.0D0*delta*delta - 20.0D0*mpi2(impi) - 11.0D0*q2 )*L & 
         + 6.0D0*S*S*D )/(216.0D0)
  ...
  FUNCTION chp_NNLO_delta_2PE_triangle_Vc_SFR(w, L, S, D, delta, impi) RESULT(res)
    ...
    REAL(8) :: res
    !const(22) used instead of 21
    res = -hA2*const(22)*delta*( 6.0D0*S*(4.0D0*c1*mpi2(impi)-2.0D0*c2*delta*delta - & 
         c3*(2.0D0*delta*delta + S))*D + &
  ...
  FUNCTION chp_NNLO_delta_2PE_single_box_Wc_SFR(w, L, S, D, delta) RESULT(res)
    ...
    REAL(8) :: res
    !const(22) used instead of 21
    res = -(b3+b8)*gA2*hA*delta*const(22)*( 6.0D0*S*S*D + & 
         (w*w-12.0D0*(delta*delta + S))*L )/(108.0D0) 
...
!!$         +c4_bar*chp_sigma_dot_q_mtx(m2,m5,q1xq3)*chp_tau1Xtau2_dot_tau3(t1,t3,t2,t4,t6,t5) ) 
    ...
    res = sigma_dot_q_ope363*&
         (tau_dot_tau_tab(t1,t3,t4,t6)*(chp_sigma_dot_q_mtx(m2,m5,qtrans1)* & 
         sum(qtrans1*qtrans3)*V3NF_F1(Q12,Aq1,impi) &
         ...
         *delta_tab(m1,m4)*delta_tab(t2,t5) &
         !
         +tau_dot_tau_tab(t2,t3,t5,t6)*( (chp_sigma_dot_q_mtx(m1,m4,qtrans1)* & 
         sum(qtrans1*qtrans3)*V3NF_F4(Aq1) &
         ...
         !
         +chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)* & 
         sigmaXsigma_dot_q(m1,m2,m4,m5,qtrans1)*V3NF_F8(Q12,Aq1,impi))
...
!123
!Eq 3.6
    res = (const(23)*gA2/2.d0)*Ct_bar*(2.d0*chp_tau_dot_tau_mtx(t1,t2,t4,t5)* & 
         chp_sigma_dot_sigma_mtx(m2,m3,m5,m6)&
...
!! N3LO 3NF Relativistic Corrections 
  ...
  function rel1PE_3nf_corr(p,q,r,s,t,u) RESULT(res)
    ...
    res = gA2/(8.d0*mnuc(imnuc)*fpi2)*chp_sigma_dot_q_mtx(m1,m4,qtrans1) & 
         /((q12+mpi2(impi))**2)*chp_tau_dot_tau_mtx(t1,t2,t4,t5)*&
         ( (1.0-2.0*beta_8)*dot_product(qtrans1,qtrans3)* & 
         (Cs_bar*chp_sigma_dot_q_mtx(m2,m5,qtrans1)*delta(t3,t6)*delta(m3,m6)&
         ...
         (2.0*imag*(2.0*beta_9+1.0)*chp_sigma_dot_q_mtx(m1,m4,kmean1)* & 
         sigmaXsigma_dot_q(m2,m3,m5,m6,qtrans1)*delta(t3,t6) &
         -(2.0*beta_9-1.0)*chp_sigma_dot_q_mtx(m1,m4,qtrans1)* & 
         chp_sigma_dot_q_mtx(m3,m6,qtrans3)*delta(m2,m5)*delta(t3,t6) &
         -2.0*imag*(2.0*beta_9-1.0)*chp_sigma_dot_q_mtx(m1,m4,qtrans1)*& 
         sigmaXsigma_dot_q(m2,m3,m5,m6,kmean2)*delta(t3,t6))&
         !
         +gA2*Cs_bar/(8.0*mnuc(0)*fpi2)*chp_tau_dot_tau_mtx(t1,t2,t4,t5)/(q12+mpi2(impi))*&
         (2.0*beta_9-1.0)*chp_sigma_dot_q_mtx(m1,m4,qtrans1)* &
         chp_sigma_dot_q_mtx(m2,m5,qtrans3)*delta(m3,m6)*delta(t3,t6)
...
!!$         chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*( chp_sigma_dot_q_mtx(m2,m5,q1xq3) + imag*dot_product(kmean2,qtrans3-qtrans1)*delta(m2,m5) )
    ...
    A123 = chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)*(&
         -gA2/(q12+mpi2(impi))*( (1-2*beta_8)*dot_product(qtrans1,qtrans3)**2*delta(m2,m5) - & 
         2*imag*chp_sigma_dot_q_mtx(m2,m5,q1xq3)*(&
         ...
         !
         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,kmean3)* & 
         (2*imag*gA2*(2*beta_9+1)*chp_sigma_dot_q_mtx(m2,m5,q1xq3))
    
    B123 = chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,qtrans3)*(&
         -gA2/(q12+mpi2(impi))*( (1-2*beta_8)*chp_sigma_dot_q_mtx(m2,m5,q1xq3)* & 
         dot_product(qtrans1,qtrans3) &
         +2*imag*dot_product(qtrans1,qtrans3)*( (1-2*beta_8)* & 
         dot_product(qtrans1,kmean2) + (1+2*beta_8)*dot_product(qtrans1,kmean1) )*delta(m2,m5))&
         !
         +2*imag*dot_product(qtrans3,kmean3-kmean2)*delta(m2,m5) + & 
         2*imag*gA2*(2*beta_9-1)*dot_product(qtrans1,kmean2)*delta(m2,m5) &
         - chp_sigma_dot_q_mtx(m2,m5,q1xq3))&
         !
         +chp_sigma_dot_q_mtx(m1,m4,qtrans1)*chp_sigma_dot_q_mtx(m3,m6,kmean3)* & 
         (-2*imag*gA2*(2*beta_9+1)*dot_product(qtrans1,qtrans3))*delta(m2,m5)

    res = gA2/(32.*mnuc(imnuc)*fpi4)*1.d0/((q12+mpi2(impi))*(q32+mpi2(impi)))*(&
         chp_tau_dot_tau_mtx(t1,t3,t4,t6)*A123*delta(t2,t5) + & 
         chp_tau1Xtau2_dot_tau3(t1,t2,t3,t4,t5,t6)*B123)
...
!123
    call ring_functions(q12,q22,q32,z13,atanQ1,atanQ2,atanQ3, & 
         Int_3pt,impi,R1,R2,R3,R4,R5,R6,r7,r8,r9,r10,r11,S1,S2,S3,s4,s5,s6,s7)
    res = 0.d0
  ...
  ! I4 = int_0^1 dxint I4int(xint)
   function I4int(q1,q3,z,impi) result(res)
     ...
     do j = 1,mesh_amount
        ...
        res = res + wpnt*( (q3**2 + q1**2*(-1.d0 + 2.d0*xint) + & 
             2.d0*q1*q3*xint*z)/sqrt(mpion2 - q1**2*(-1.d0 + xint)*xint) &
             + (q1**2 + q3**2*(-1 + 2.d0*xint) + 2.d0*q1*q3*xint*z)/ & 
             sqrt(mpion2 - q3**2*(-1.d0 + xint)*xint))&
             /(16.*PI*(4.d0*(mpion2 - q1**2*(-1.d0 + xint)*xint)*(q1**2 + q3**2 + 2.d0*q1*q3*z) & 
             +(q3**2 + q1**2*(-1.d0 + 2.d0*xint) + 2.d0*q1*q3*xint*z)**2.d0))
     end do
     
   end function I4int

   subroutine ring_functions(qtrans12,qtrans22,qtrans32,z,arctanQ1, & 
        arctanQ2,arctanQ3,ringInt,impi,R1,R2,R3,R4,R5,R6,r7,r8,r9,r10,r11,&
     ...
     R1=((mpion*(2.d0*mpion2 + q32)*(-1.d0 + z2)*(4.d0*mpion2*(q3 + q1*z) + q3*q22))&
          ...
          !
          - (Int_3pt*q22*(q3*q22*(-q32 + q12*z2 + q1*q3*z*(-1.d0 + z2)) + & 
          8*mpion4*(-1.d0 + z2)*(2*q1*z + q3*(1.d0 + z2)) &
          + 2*mpion2*(q13*z*(-2 + z2) + 3*q1*q32*z*(-2 + z2) - q12*q3*(1 + 2*z2) + q33*(-3 + 2*z**4))))&
          /(32.d0*q3*(-1.d0 + z2)*(-q22 + 4*mpion2*(-1.d0 + z2))) &
          + ((2.d0*mpion2*q22 + q3*(q33 - q13*z + q12*q3*(2 - 3*z2) - & 
          q1*q32*z*(-2.d0 + z2)))*atanQ1)/(256.d0*pi*q1*q32*(-1.d0 + z2)) &
          !
          - ((q3*z*(-q3 + q1*z)*q22 + 2.d0*mpion2*(-(q32*z) + q12*z*(-2.d0 + z2) - & 
          q1*q3*(1 + z2)))*atanQ3)/(256.d0*pi*q1*q32*(-1.d0 + z2))&
     ...
     R2 = (mpion*(2*mpion2 +q32)*(4*mpion2*(q3 + q1*z) + q3*q22))&
          ...
          + q12*q3*(1 + 2*z2)**2 + q1*q32*z*(2 + 7*z2)) + & 
          2*mpion2*q22*(2*q13*z + q33*(3 + 3*z2 - 4*z2**2)&
          - 2*q1*q32*z*(-1 - 3*z2 + z2**2) + q12*q3*(1 -z2 + 6*z2**2))))&
          /(32.*q12*q3*(-1.d0 + z2)**2*(q22 - 4*mpion2*(-1.d0 + z2)))&
          !
          + ((2*mpion2*(q12*(1 + z2) + q32*(1 + z2) + q1*q3*z*(3 + z2)) + & 
          q3*(q33*(1 + z2) + q1*q32*z*(1 + z2)&
    ...
    R3 = ((mpion*(2*mpion2 + q32)*z*(4*mpion2*(q3 + q1*z) + q3*q22))&
         ...
         !
         - (Int_3pt*z*(q3*q22**2*(q1*q3*z*(-1.d0 + z2) + & 
         q12*(1 + z2) - q32*(1 + z2)) &
         + 8*mpion4*(-1 + z)*(1 + z)*(3*q13*z + q33*(1 + 2*z2) + &
         3*q1*q32*z*(1 + 2*z2) + q12*q3*(-1 + 10*z2)) &
         + 2*mpion2*q22*(q12*q3*(3 - 9*z2) + q13*z*(-3 + z2) - & 
         q1*q32*z*(5 + z2) + q33*(-3 - 3*z2 + 4*z**4))))&
         /(32.*q1*q32*(-1+z2)**2*(q22 - 4*mpion2*(-1.d0 + z2))) &
         !
         - (z*(2*mpion2*(2*q12 + 4*q1*q3*z + q32*(1 + z2)) + & 
         q3*(-2*q13*z + 2*q1*q32*z + q12*q3*(1 - 3*z2) + q33*(1 + z2)))*atanQ1)&
         /(256.*pi*q12*q33*(-1+z2)**2) &
         !
         - (z*(mpion2*(4*q32*z - 2*q12*z*(-3 + z2) + 4*q1*q3*(1 + z2)) + & 
         q3*(2*q33*z - 2*q12*q3*z**3 + q13*(-1 - z2) + q1*q32*(1 + z2)))*atanQ3)&
         /(256.*pi*q12*q33*(-1+z2)**2) &
         !
         - (z*q2*(-4*mpion2*(q1 + q3*z) + q3*(2*q12*z - 2*q32*z + &
         q1*q3*(-1.d0 + z2)))*atanQ2)/(256.*PI*q12*q33*(-1+z2)**2));
    
    R4 = (mpion*(2*mpion2 + q32)*z*(4*mpion2*(q3 + q1*z) + q3*q22))&
         /(128.*pi*q1*q32*(4*mpion2 +q32)* (q22 - 4*mpion2*(-1 +z2))) &
         !
         - (Int_3pt*(q3*q22**2*(-2*q32*z + q1*q3*(-1.d0 + z2)**2 + & 
         q12*(z + z**3)) &
         + 8*mpion4*(-1 + z)*(1 + z)*(3*q13*z2 + 9*q12*q3*z**3 + &
         q33*z*(2 + z2) + q1*q32*(-2 + 9*z2+ 2*z2**2)) &
         + 2*mpion2*q22*(q13*z2*(-3 + z2) + q12*q3*(2*z - 8*z**3) + &
         2*q33*z*(-3 +z2 + z2**2) &
         + q1*q32*(4 + 5*z2*(-3 + z2)))))/(32.*q1*q32*(-1.d0 + z2)**2* &
         (q22 - 4.d0*mpion2*(-1.d0 + z2))) &
         !
         + ((-2.d0*mpion2*(2.d0*q12*z + 2.d0*q32*z + q1*q3*(1 + 3*z2)) + &
         q3*(-2*q33*z + 2*q13*z2 + 2*q12*q3*z**3&
         + q1*q32*(1 - 4*z2 + z2**2)))*atanQ1)/(256.*pi*q12*q33*(-1.d0 + z2)**2)&
         !
         - ((2*mpion2*(-(q12*z2*(-3 + z2)) + q32*(1 + z2) + q1*q3*z*(3 +z2))&
         + q3*(q33*(1 +z2) &
         + q1*q32*z*(1 +z2) + q13*(-z - z**3) - & 
         q12*q3*(1 -z2 + 2*z2**2)))*atanQ3)/(256.*pi*q12*q33*(-1.d0 + z2)**2)&
         !
   ...
   R5 = -(mpion*(2*mpion2 + q32)*(4*mpion2*(q3 + q1*z) + q3*q22))&
         ...
         !
         - ((2*mpion2*(-2*q32*z + q12*z*(-3 +z2) - 2*q1*q3*(1 + z2)) + & 
         q3*(-2*q33*z + 2*q12*q3*z**3 +q13*(1 +z2) &
    ...
    R6 =  -(Int_3pt*(2*mpion2 +q22)*(q1*q3*(q12 +q32 + q1*q3*z)*q22 &
         + 2*mpion2*(4*q1*q3*(q12 +q32) + (q1**4 + 6*q12*q3**2 +q3**4)*z) + & 
         4*mpion4*(q12*z + q32*z &
         ...
         + 2*mpion4*(q13*q3*(77 - 36*z2) + q1*q33*(77 - 36*z2) + & 
         4*q1**4*z*(-1 +z2) + 4*q3**4*z*(-1 +z2) &
    ...
    R7 =  (3*mpion*(2*mpion2 +q22))/(256.*pi*q12*(q22 - 4*mpion2*(-1 +z2)))&
         ...
         + q1*q3*z*(3 +z2)) + 4*mpion2*(-1 +z2)*(2*q1*q3*z*(2 +z2) + &
         q12*(1 + 2*z2) +q32*(1 + 2*z2))))/(64.*q12*(-1.d0 + z2)**2.* &
         (-q12 -q32 - 2.*q1*q3*z + 4.*mpion2*(-1. +z2)))&
         !
...
!         
         - (3*Int_3pt*z*(2*mpion2 +q22)*((-q12 -q32 - 2*q1*q3*z)* & 
         (q12*(1 +z2) +q32*(1 + z2) + q1*q3*z*(3 + z2))&
         + 4*mpion2*(-1 +z2)*(2*q1*q3*z*(2 +z2) +q12*(1 + 2*z2) + & 
         q32*(1 + 2*z2))))/(64.*q1*q3*(-1.d0 + z2)**2* & 
         (-q12 -q32 - 2*q1*q3*z + 4*mpion2*(-1.d0 + z2)))&
         !
         + (3*z*(2*mpion2 +q1**2 +q32 + 2*q1*q3*z)*(2*q1*z + q3*(1 + z2))* & 
         atanQ1)/(512.*pi*q12*q32*(-1.d0 + z2)**2)&
!
    ...
    R9 =  ((-3*mpion*z*(2*mpion2 + q22))/(256.*pi*q1*q3*(q22 - 4*mpion2*(-1.d0 + z2))) &
         !
         + (3*Int_3pt*z*(2*mpion2 + q22)*(q22*(-2*q12 - 2*q32 + & 
         q1*q3*z*(-5 + z2)) &
         + 4*mpion2*(-1.d0 + z2)*(6*q1*q3*z + q12*(2 + z2) + q32*(2 + z2))))/ & 
         (64.*q1*q3*(-1.d0 + z2)**2*(q22 - 4*mpion2*(-1.d0 + z2))) &
         !
         + (3*(2*q33*z - q1*q32*z2*(-7 + z2) + q13*(1 + z2) + 2*q12*q3*z*(2 + z2) &
         + 2*mpion2*(2*q3*z + q1*(1 + z2)))*atanQ1)&
         /(512.*pi*q12*q32*(-1.d0 + z2)**2) &
         !
         + (3*(2*q13*z - q12*q3*z2*(-7 + z2) + q33*(1 + z2) + 2*q1*q32*z*(2 + z2) + & 
         2*mpion2*(2*q1*z + q3*(1 + z2)))*atanQ3)/(512.*pi*q12*q32*(-1.d0 + z2)**2) &
         !
    ...
    R10 = (3*mpion*(2*mpion2 + q22)*(-1.d0 + z2))/(256.*pi*(q22 - 4*mpion2*(-1.d0 + z2)))&
         !
         + (3*Int_3pt*(2*mpion2 + q22)*((-q22)*(q12 + q32 - q1*q3*z*(-3 + z2)) + &
         4*mpion2*(-1.d0 + z2)&
         *(4*q1*q3*z + q12*(1 + z2) + q32*(1 + z2))))/(64.*(-1.d0 + z2)* & 
         (-q22 + 4*mpion2*(-1.d0 + z2)))&
         !
         - (3*(q33 + q13*z + 2*mpion2*(q3 + q1*z) - q1*q32*z*(-4 + z2) + &
         q12*q3*(1 + 2*z2))*atanQ1)/(512.*pi*q1*q3*(-1.d0 + z2)) &
         !
         - (3*(q13 + q33*z + 2*mpion2*(q1 + q3*z) - q12*q3*z*(-4 + z2) + &
         q1*q32*(1 + 2*z2))*atanQ3)/(512.*pi*q1*q3*(-1.d0 + z2)) &
         !
         + (3*(q3+q1*z)*(q1+q3*z)*(2*mpion2+q12 +q32 +2*q1*q3*z)*atanQ2)&
         /(512.*pi*q1*q3*q2*(-1.d0 + z2))

    R11 =  (-(mpion*(2*mpion2*(4*mpion2 + q12 + q32)*(q12*q32 + 2*mpion2*(q12 + q32)) &
         - q1*q3*(32*mpion**6 + 12*mpion**4*(q12 + q32)&
         + q12*q32*(q12 + q32) + 2*mpion2*(q1**4 + 4*q12*q32 +q3**4))*z - 2* &
         (4*mpion**4*q12*(4*mpion2 + q12)&
         + 4*mpion2*(2*mpion2 + q12)**2*q32 +(2*mpion2 + q12)**2*q3**4)*z2))/&
         (256.*pi*q12*(4*mpion2 + q12)*q32*(4*mpion2 + q32)*(q22 - 4*mpion2*(-1.d0 + z2)))&
         !
         - (Int_3pt*q22*((-2*mpion2 - q12)*(2*mpion2 + q32)*(4*mpion2 + q12 + q32) &
         + q1*q3*(8*mpion4 +q1**4 +q3**4 &
         + 4*mpion2*(q12 + q32))*z + (4*mpion2 + q12 + q32)* &
         (4*mpion4 + 3*q12*q32 + 2*mpion2*(q12 + q32))*z2 &
         + 2*q1*q3*(-4*mpion4 + q12*q32)*z**3))/(64.*q12*q32*&
         (-1.d0 + z2)*(q22 - 4*mpion2*(-1.d0 + z2))) &
         !
         + ((2*mpion2*(2*q12 + 2*q1*q3*z + q32*(-1.d0 + z2)) + &
         q1*(q13 + q12*q3*z + q33*z + q1*q32*(-1 + 2*z2)))*atanQ1)/(512.*pi*q13*q32*(-1.d0 + z2)) &
         !
         + ((2*mpion2*(2*q32 + 2*q1*q3*z + q12*(-1.d0 + z2)) + &
         q3*(q33 + q13*z + q1*q32*z + q12*q3*(-1 + 2*z2)))*atanQ3)/(512.*pi*q12*q33*(-1.d0 + z2)) &
         !
    ...
    S2 = ((Int_3pt*q3*(2*q1**2*z + 2*q32*z - 4*mpion2*z*(-1.d0 + z2) + &
         q1*q3*(1 + 3*z2)))/(16.*q1*(-1.d0 + z2)**2) &
    ...
    S3 = (-(Int_3pt*(-4*mpion2*(-1.d0 + z2) + q12*(1 + z2) + q32*(1 + z2) + &
         q1*q3*z*(3 + z2)))/(16.*(-1+z2)**2) &
    ...
    S4 =((Int_3pt*z*(-2*q12*z - 2*q32*z + 4*mpion2*z*(-1.d0 + z2) - &
         q1*q3*(1 + 3*z2)))/(16.*(-1+z2)**2) &
    ...
    S5 = ((Int_3pt*q1*(2*q12*z + 2*q32*z - 4*mpion2*z*(-1.d0 + z2) + &
         q1*q3*(1 + 3*z2)))/(16.*q3*(-1+z2)**2) &
   ...
   function R1_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
     ...
     R1_3nf=((mpion*(2*mpion2 + q32)*(-1 + z2)*(4*mpion2*(q3 + q1*z) + &
          q3*(q12 + q32 + 2*q1*q3*z)))&
          /(128.*pi*(4*mpion2*q3 + q33)*(-q12 - q32 - 2*q1*q3*z + 4*mpion2*(-1 + z2))) &
!
          - (Int_3pt*(q12 + q32 + 2*q1*q3*z)*(q3*(q12 + q32 + 2*q1*q3*z)* &
          (-q32 + q12*z2 + q1*q3*z*(-1 + z2)) + 8*mpion4*(-1 + z2)*(2*q1*z + q3*(1 + z2)) &
          + 2*mpion2*(q13*z*(-2 + z2) + 3*q1*q32*z*(-2 + z2) - q12*q3*(1 + 2*z2) + & 
          q33*(-3 + 2*z**4))))&
          /(32.*q3*(-1 + z2)*(-q12 - q32 - 2*q1*q3*z + 4*mpion2*(-1 + z2))) &
          + ((2*mpion2*(q12 + q32 + 2*q1*q3*z) + q3*(q33 - q13*z + q12*q3*(2 - 3*z2) - &
          q1*q32*z*(-2 + z2)))*atan(q1/(2.*mpion)))/(256.*pi*q1*q32*(-1 + z2)) &
!
          - ((q3*z*(-q3 + q1*z)*(q12 + q32 + 2*q1*q3*z) + 2*mpion2*(-(q32*z) + q12*z*(-2 + z2) - &
          q1*q3*(1 + z2)))*atan(q3/(2.*mpion)))/(256.*pi*q1*q32*(-1 + z2))&
          - (sqrt(q12 + q32 + 2*q1*q3*z)*(q3*(-q12 + q32)*z + 2*mpion2*(q1 + q3*z))* &
          atan(sqrt(q12 + q32 + 2*q1*q3*z)/(2.*mpion)))/(256.*pi*q1*q32*(-1 + z2)));
...
!!$    coeff = coeff/128.d0
    ...
    r2_3nf = (mpion*(2*mpion2 +q32)*(4*mpion2*(q3 + q1*z) + q3*(q12 + q32 + 2*q1*q3*z)))&
         ...
         - 8*mpion4*(-1 + z)*(1 + z)*(q13*z*(2 + z2) +q33*(1 + 2*z2)&
         + q12*q3*(1 + 2*z2)**2 + q1*q32*z*(2 + 7*z2)) + & 
         2*mpion2*(q12 + q32 + 2*q1*q3*z)*(2*q13*z + q33*(3 + 3*z2 - 4*z2**2)&
         - 2*q1*q32*z*(-1 - 3*z2 + z2**2) + q12*q3*(1 -z2 + 6*z2**2))))&
         /(32.*q12*q3*(-1 + z2)**2*(q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2)))&
         !
         + ((2*mpion2*(q12*(1 + z2) + q32*(1 + z2) + q1*q3*z*(3 + z2)) + &
         q3*(q33*(1 + z2) + q1*q32*z*(1 + z2)&
         + q13*(-z - z**3) + q12*q3*(2 - 5*z2 + z2**2)))*atan(q1/(2.*mpion)))/&
         (256.*pi*q13*q32*(-1 + z2)**2)&
         !
         + ((q3*z*(q33 - q13*z + q1*q32*z - q12*q3*z2) + mpion2*(2*q12*z + 2*q32*z &
         + q1*q3*(1 + 3*z2)))*atan(q3/(2.*mpion)))/(128.*pi*q13*q32*(-1 + z2)**2) &
         ! 
         + (sqrt(q12 +q32+ 2*q1*q3*z)*(-2*mpion2*(2*q3*z + q1*(1 + z2)) &
         + q3*z*(-2*q32 +q12*(1 + z2)))*atan(sqrt(q12 + q32 + 2*q1*q3*z)/ &
         (2.*mpion)))/(256.*pi*q13*q32*(-1 + z2)**2);
  ...
  function R3_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    R3_3nf = ((mpion*(2*mpion2 + q32)*z*(4*mpion2*(q3 + q1*z) + q3*(q12 + q32 + 2*q1*q3*z)))&
         ...
         !
         - (Int_3pt*z*(q3*(q12 + q32 + 2*q1*q3*z)**2*(q1*q3*z*(-1 + z2) + &
         q12*(1 + z2) - q32*(1 + z2)) &
         + 8*mpion4*(-1 + z)*(1 + z)*(3*q13*z + q33*(1 + 2*z2) + & 
         3*q1*q32*z*(1 + 2*z2) + q12*q3*(-1 + 10*z2)) &
         + 2*mpion2*(q12 + q32 + 2*q1*q3*z)*(q12*q3*(3 - 9*z2) + q13*z*(-3 + z2) - &
         q1*q32*z*(5 + z2) + q33*(-3 - 3*z2 + 4*z**4))))&
         /(32.*q1*q32*(-1+z2)**2*(q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2))) &
         !
         - (z*(2*mpion2*(2*q12 + 4*q1*q3*z + q32*(1 + z2)) + &
         q3*(-2*q13*z + 2*q1*q32*z + q12*q3*(1 - 3*z2) + q33*(1 + z2)))*atan(q1/(2.*mpion)))&
         /(256.*pi*q12*q33*(-1+z2)**2) &
         !
         - (z*(mpion2*(4*q32*z - 2*q12*z*(-3 + z2) + 4*q1*q3*(1 + z2)) + &
         q3*(2*q33*z - 2*q12*q3*z**3 + q13*(-1 - z2) + q1*q32*(1 + z2)))*atan(q3/(2.*mpion)))&
         /(256.*pi*q12*q33*(-1+z2)**2) &
         !
         - (z*sqrt(q12 + q32 + 2*q1*q3*z)*(-4*mpion2*(q1 + q3*z) + &
         q3*(2*q12*z - 2*q32*z + q1*q3*(-1 + z2)))*atan(sqrt(q12 + q32 + 2*q1*q3*z)/&
         (2.*mpion)))/(256.*PI*q12*q33*(-1+z2)**2));
  ...
  function R4_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    R4_3nf = (mpion*(2*mpion2 + q32)*z*(4*mpion2*(q3 + q1*z) + q3*(q12 + q32 + 2*q1*q3*z)))&
         /(128.*pi*q1*q32*(4*mpion2 +q32)* (q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 +z2))) &
         !
         - (Int_3pt*(q3*(q12 + q32 + 2*q1*q3*z)**2* &
         (-2*q32*z + q1*q3*(-1 + z2)**2 + q12*(z + z**3)) &
         + 8*mpion4*(-1 + z)*(1 + z)*(3*q13*z2 + 9*q12*q3*z**3 &
         +q33*z*(2 + z2) + q1*q32*(-2 + 9*z2+ 2*z2**2)) &
         + 2*mpion2*(q12 + q32 + 2*q1*q3*z)*(q13*z2*(-3 + z2) + &
         q12*q3*(2*z - 8*z**3) + 2*q33*z*(-3 +z2 + z2**2) &
         + q1*q32*(4 + 5*z2*(-3 + z2)))))/(32.*q1*q32*(-1 + z2)**2* &
         (q12 +q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2))) &
         !
         + ((-2*mpion2*(2*q12*z + 2*q32*z + q1*q3*(1 + 3*z2)) + & 
         q3*(-2*q33*z + 2*q13*z2 + 2*q12*q3*z**3&
         + q1*q32*(1 - 4*z2 + z2**2)))*atan(q1/(2.*mpion)))/(256.*pi*q12*q33*(-1 + z2)**2)&
         !
         - ((2*mpion2*(-(q12*z2*(-3 + z2)) + q32*(1 + z2) + q1*q3*z*(3 +z2)) + q3*(q33*(1 +z2) &
         + q1*q32*z*(1 +z2) + q13*(-z - z**3) - q12*q3*(1 -z2 + 2*z2**2)))* & 
         atan(q3/(2.*mpion)))/(256.*pi*q12*q33*(-1 + z2)**2)&
         !
...
!    R4_3nf = R4_3nf/(hc**7.0)
  ...
  function R5_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    R5_3nf = -(mpion*(2*mpion2 + q32)*(4*mpion2*(q3 + q1*z) + q3*(q12 + q32 + 2*q1*q3*z)))&
         ...
         + 8*mpion4*(-1 + z)*(1 + z)*(3*q13*z + q33*(1 + 2*z2) + &
         3*q1*q32*z*(1 + 2*z2) + q12*q3*(-1 + 10*z2)) &
         + 2*mpion2*(q12 +q32 + 2*q1*q3*z)*(q12*q3*(3 - 9*z2) + &
         q13*z*(-3 + z2) - q1*q32*z*(5 + z2) &
         + q33*(-3 - 3*z2 + 4*z2**2))))/(32.*q33*(-1 +z2)**2* &
         (q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2)))&
         !
         + ((2*mpion2*(2*q12 + 4*q1*q3*z + q32*(1 + z2)) + q3*(-2*q13*z + 2*q1*q32*z &
         ...
         + q1*q3*(-1 + z2)))*atan(sqrt(q12 +q32 + 2*q1*q3*z)/(2.*mpion)))/ &
         (256.*pi*q1*q3**4*(-1 +z2)**2);
...
!!$    coeff = coeff/128.d0
  ...
  function R7_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    R7_3nf =  (3*mpion*(2*mpion2 +q12 +q32 + 2*q1*q3*z))/ & 
         (256.*pi*q12*(q12 +q32 + 2*q1*q3*z - 4*mpion2*(-1 +z2)))&
         !
         + (3*Int_3pt*(2*mpion2 +q12 +q32 + 2*q1*q3*z)* & 
         ( (-q12 -q32 - 2*q1*q3*z)*(q12*(1 +z2) +q32*(1 +z2) &
         + q1*q3*z*(3 +z2)) + 4*mpion2*(-1 +z2)*(2*q1*q3*z*(2 +z2) +q12*(1 + 2*z2) +q32*(1 + 2*z2)))) & 
         /(64.*q12*(-1 + z2)**2.*(-q12 -q32 - 2.*q1*q3*z + 4.*mpion2*(-1. +z2)))&
         !
         - (3*(2*mpion2 +q12 +q32 + 2*q1*q3*z)*(2*q1*z + q3*(1 +z2))* & 
         atan(q1/(2.*mpion)))/(512.*pi*q13*q3*(-1 +z2)**2.) &
         !
         - (3*(2*mpion2 +q12 +q32 + 2*q1*q3*z)*(2*q3*z + q1*(1 +z2))*& 
         atan(q3/(2.*mpion)))/(512.*pi*q13*q3*(-1 +z2)**2.) &
         !
         + (3*(2*mpion2 +q12 +q32 + 2*q1*q3*z)*(2*q12*z + 2*q32*z + q1*q3*(1 + 3*z2))* & 
         atan(sqrt(q12 + q32 + 2*q1*q3*z)/(2.*mpion)))&
  ...
  function R8_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    R8_3nf = (-3*mpion*z*(2*mpion2 +q12 +q32 + 2*q1*q3*z))/ & 
         (256.*pi*q1*q3*(q12 +q32 + 2*q1*q3*z - 4*mpion2*(-1 +z2))) &
!         
         - (3*Int_3pt*z*(2*mpion2 +q12 + q32 + 2*q1*q3*z)* & 
         ((-q12 -q32 - 2*q1*q3*z)*(q12*(1 +z2) +q32*(1 + z2) + q1*q3*z*(3 + z2))&
         + 4*mpion2*(-1 +z2)*(2*q1*q3*z*(2 +z2) +q12*(1 + 2*z2) + q32*(1 + 2*z2))))/& 
         (64.*q1*q3*(-1 + z2)**2*(-q12 -q32 - 2*q1*q3*z + 4*mpion2*(-1 + z2)))&
         !
         + (3*z*(2*mpion2 +q1**2 +q32 + 2*q1*q3*z)*(2*q1*z + q3*(1 + z2))* & 
         atan(q1/(2.*mpion)))/(512.*pi*q12*q32*(-1 + z2)**2)&
!
         + (3*z*(2*mpion2 +q12 +q32 + 2*q1*q3*z)*(2*q3*z + q1*(1 +z2))* & 
         atan(q3/(2.*mpion)))/(512.*pi*q12*q32*(-1 +z2)**2)&
         !
         - (3*z*(2*mpion2 + q12 +q32 + 2*q1*q3*z)*(2*q12*z + 2*q32*z + q1*q3*(1 + 3*z2))* & 
         atan(sqrt(q12 +q32 + 2*q1*q3*z)/(2.*mpion)))&
  ...
  function R9_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    R9_3nf=  ((-3*mpion*z*(2*mpion2 + q12 + q32 + 2*q1*q3*z))/ & 
         (256.*pi*q1*q3*(q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2))) &
         !
         + (3*Int_3pt*z*(2*mpion2 + q12 + q32 + 2*q1*q3*z)* & 
         ((q12 + q32 + 2*q1*q3*z)*(-2*q12 - 2*q32 + q1*q3*z*(-5 + z2)) &
         + 4*mpion2*(-1 + z2)*(6*q1*q3*z + q12*(2 + z2) + q32*(2 + z2))))/& 
         (64.*q1*q3*(-1 + z2)**2*(q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2))) &
         !
         + (3*(2*q33*z - q1*q32*z2*(-7 + z2) + q13*(1 + z2) + & 
         2*q12*q3*z*(2 + z2) + 2*mpion2*(2*q3*z + q1*(1 + z2)))*atan(q1/(2.*mpion)))&
         /(512.*pi*q12*q32*(-1 + z2)**2) &
         !
         + (3*(2*q13*z - q12*q3*z2*(-7 + z2) + q33*(1 + z2) + 2*q1*q32*z*(2 + z2) + & 
         2*mpion2*(2*q1*z + q3*(1 + z2)))*atan(q3/(2.*mpion)))/(512.*pi*q12*q32*(-1 + z2)**2) &
         !
         - (3*(2*mpion2 + q12 + q32 + 2*q1*q3*z)*(q12*(1 + z2) + q32*(1 + z2) + q1*q3*z*(3 + z2))* & 
         atan(sqrt(q12 + q32 + 2*q1*q3*z)/(2.*mpion)))&
         /(512.*pi*q12*q32*sqrt(q12 + q32 + 2*q1*q3*z)*(-1 + z2)**2));
  ...
  function R10_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    R10_3nf = (3*mpion*(2*mpion2 + q12 + q32 + 2*q1*q3*z)*(-1 + z2))/ & 
         (256.*pi*(q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2)))&
         !
         + (3*Int_3pt*(2*mpion2 + q12 + q32 + 2*q1*q3*z)* & 
         ((-q12 - q32 - 2*q1*q3*z)*(q12 + q32 - q1*q3*z*(-3 + z2)) + 4*mpion2*(-1 + z2)&
         *(4*q1*q3*z + q12*(1 + z2) + q32*(1 + z2))))/ & 
         (64.*(-1 + z2)*(-q12 - q32 - 2*q1*q3*z + 4*mpion2*(-1 + z2)))&
         !
         - (3*(q33 + q13*z + 2*mpion2*(q3 + q1*z) - q1*q32*z*(-4 + z2) + & 
         q12*q3*(1 + 2*z2))*atan(q1/(2.*mpion)))/(512.*pi*q1*q3*(-1 + z2)) &
         !
         - (3*(q13 + q33*z + 2*mpion2*(q1 + q3*z) - q12*q3*z*(-4 + z2) + & 
         q1*q32*(1 + 2*z2))*atan(q3/(2.*mpion)))/(512.*pi*q1*q3*(-1 + z2)) &
         !
         + (3*(q3+q1*z)*(q1+q3*z)*(2*mpion2+q12 +q32 +2*q1*q3*z)* & 
         atan(sqrt(q12 + q32 + 2*q1*q3*z)/(2.*mpion)))&
  ...
  function R11_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    R11_3nf =  (-(mpion*(2*mpion2*(4*mpion2 + q12 + q32)* & 
         (q12*q32 + 2*mpion2*(q12 + q32)) - q1*q3*(32*mpion**6 + 12*mpion**4*(q12 + q32)&
         + q12*q32*(q12 + q32) + 2*mpion2*(q1**4 + 4*q12*q32 +q3**4))*z - & 
         2*(4*mpion**4*q12*(4*mpion2 + q12)&
         + 4*mpion2*(2*mpion2 + q12)**2*q32 +(2*mpion2 + q12)**2*q3**4)*z2))/ & 
         (256.*pi*q12*(4*mpion2 + q12)*q32*(4*mpion2 + q32)* & 
         (q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2)))&
!
         - (Int_3pt*(q12 + q32 + 2*q1*q3*z)*((-2*mpion2 - q12)* & 
         (2*mpion2 + q32)*(4*mpion2 + q12 + q32) + q1*q3*(8*mpion4 +q1**4 +q3**4 &
         + 4*mpion2*(q12 + q32))*z + (4*mpion2 + q12 + q32)* & 
         (4*mpion4 + 3*q12*q32 + 2*mpion2*(q12 + q32))*z2 &
         + 2*q1*q3*(-4*mpion4 + q12*q32)*z**3))/(64.*q12*q32*(-1 + z2)* & 
         (q12 + q32 + 2*q1*q3*z - 4*mpion2*(-1 + z2))) &
!
         + ((2*mpion2*(2*q12 + 2*q1*q3*z + q32*(-1 + z2)) + & 
         q1*(q13 + q12*q3*z + q33*z + q1*q32*(-1 + 2*z2)))*atan(q1/(2.*mpion)))/(512.*pi*q13*q32*(-1 + z2)) &
         !
         + ((2*mpion2*(2*q32 + 2*q1*q3*z + q12*(-1 + z2)) +  & 
         q3*(q33 + q13*z + q1*q32*z + q12*q3*(-1 + 2*z2)))*atan(q3/(2.*mpion)))/(512.*pi*q12*q33*(-1 + z2)) &
!
         - ((4*mpion2 + q12 + q32)*sqrt(q12 + q32 + 2*q1*q3*z)* & 
         atan(sqrt(q12 + q32 + 2*q1*q3*z)/(2.*mpion)))/(512.*pi*q12*q32*(-1 + z2)));
  ...
  function S1_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    S1_3nf = (-mpion/(32.*pi) + (Int_3pt*(2*mpion2 + q12)*(2*mpion2 + q32))/(16.) &
         ...
         - ((4*mpion2 + q12 + q32 + q1*q3*z)*atan(sqrt(q12 + q32 + 2*q1*q3*z)/ & 
         (2.*mpion)))/(128.*pi*sqrt(q12 + q32 + 2*q1*q3*z)))/2.;
  ...
  function S2_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
    ...
    S2_3nf = ((Int_3pt*q3*(2*q1**2*z + 2*q32*z - & 
         4*mpion2*z*(-1 + z2) + q1*q3*(1 + 3*z2)))/(16.*q1*(-1 + z2)**2) &
         - ((2*q3*z + q1*(1 + z2))*atan(q1/(2.*mpion)))/(128.*pi*q1**2*(-1 + z2)**2) &
         - ((2*q1*z + q3*(1 + z2))*atan(q3/(2.*mpion)))/(128.*pi*q1**2*(-1 + z2)**2) &
         + ((q1**2*(1 + z2) + q32*(1 + z2) + q1*q3*z*(3 + z2))* & 
         atan(sqrt(q1**2 + q32 + 2*q1*q3*z)/(2.*mpion)))&
...
function S3_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
  ...
  S3_3nf = 0d0
  S3_3nf= (-(Int_3pt*(-4*mpion2*(-1 + z2) + q12*(1 + z2) + q32*(1 + z2) + &
       q1*q3*z*(3 + z2)))/(16.*(-1+z2)**2) &
       + ((2*q1*z + q3*(1 + z2))*atan(q1/(2.*mpion)))/(128.*pi*q1*q3*(-1+z2)**2) &
       + ((2*q3*z + q1*(1 + z2))*atan(q3/(2.*mpion)))/(128.*pi*q1*q3*(-1+z2)**2) &
       + (z*(-2*q12 - 2*q32 + q1*q3*z*(-5 + z2))*atan(sqrt(q12 + q32 + 2*q1*q3*z)/ & 
       (2.*mpion)))/(128.*pi*q1*q3*sqrt(q12 + q32 + 2*q1*q3*z)*(-1+z2)**2))/2.;
...
function S4_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
  ...
  S4_3nf=((Int_3pt*z*(-2*q12*z - 2*q32*z + 4*mpion2*z*(-1 + z2) - & 
       q1*q3*(1 + 3*z2)))/(16.*(-1+z2)**2) &
       + (z*(2*q3*z + q1*(1 + z2))*atan(q1/(2.*mpion)))/(128.*pi*q1*q3*(-1+z2)**2) &
       + (z*(2*q1*z + q3*(1 + z2))*atan(q3/(2.*mpion)))/(128.*pi*q1*q3*(-1+z2)**2)&
       - (z*(q12*(1 + z2) + q32*(1 + z2) + q1*q3*z*(3 + z2))*atan(sqrt(q12 + q32 + 2*q1*q3*z)/ & 
       (2.*mpion)))/(128.*pi*q1*q3*sqrt(q12 + q32 + 2*q1*q3*z)*(-1+z2)**2))/2.;
...
function S5_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
  ...
  S5_3nf= ((Int_3pt*q1*(2*q12*z + 2*q32*z - 4*mpion2*z*(-1 + z2) + q1*q3*(1 + 3*z2)))/(16.*q3*(-1+z2)**2) &
       ...
       + ((q12*(1 + z2) + q32*(1 + z2) + q1*q3*z*(3 + z2))* & 
       atan(sqrt(q12 + q32 + 2*q1*q3*z)/(2.*mpion)))/(128.*pi*q32*sqrt(q12 + q32 + 2*q1*q3*z)*(-1+z2)**2))/2.
...
function S6_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
  ...
  S6_3nf = ((Int_3pt*q1*q3*(q3 + q1*z)*(q1 + q3*z))/(16.*(-1 + z2)) &
       ...
       + ((q12 + q32 - q1*q3*z*(-3 + z2))*atan(sqrt(q12 + q32 + 2*q1*q3*z)/ & 
       (2.*mpion)))/(128.*pi*sqrt(q12 + q32 + 2*q1*q3*z)*(-1 + z2)))/2.
...
function S7_3nf(qtrans12,qtrans32,z,ringInt,impi,eps)
  ...
  S7_3nf= (-(Int_3pt*(2*mpion2 + q32)*(q3 + q1*z))/(32.*q3*(-1 + z2)) &
       ...
       - ((q32*z*(q3 + q1*z) + 2*mpion2*(q1 + q3*z))*atan(sqrt(q12 + q32 + 2*q1*q3*z)/ &
       (2.*mpion)))/(256.*pi*q1*q32*sqrt(q12 + q32 + 2*q1*q3*z)*(-1 + z2)))/2.;
...
!!$    if ( nz1 + nz2 /= nz3 + nz4 ) return 
  ...
  complex*16 function chiral_3nf_asym(i,j,p,k,l,q)
    ...
    INTEGER, intent(in) :: i,j,p,k,l,q
    if(chiral_delta_flag) then
       chiral_3nf_asym = chiral_3nf_with_delta_asym(i,j,p,k,l,q)
  ...
  complex*16 function chiral_3nf(p,q,r,s,t,u)
    ...
    vn3lo_2PE = & 
         ...
         (tau_dot_tau_tab(t1,t3,t4,t6)*(mpi(impi)*(mpi2(impi) + 3.d0*Q12 + & 
         3.d0*Q32 + 4.d0*sum(qtrans1*qtrans3)) &
         +(2.d0*mpi2(impi) + Q12 + Q32 + 2.d0*sum(qtrans1*qtrans3) ) &
         *(3.d0*mpi2(impi) + 3.d0*Q12 + 3.d0*Q32 + 4.d0*sum(qtrans1*qtrans3)) * Aq2) &
         *delta_tab(m2,m5)*delta_tab(t2,t5) &
         ...
         *(mpi(impi)+(4.d0*mpi2(impi)+Q12+Q32+2.d0*sum(qtrans1*qtrans3))*Aq2))+&
         ...
         (tau_dot_tau_tab(t2,t3,t5,t6)*(mpi(impi)*(mpi2(impi) + 3.d0*Q22 + & 
         3.d0*Q32 + 4.d0*sum(qtrans2*qtrans3)) &
         ...
         *(mpi(impi)+(4.d0*mpi2(impi)+Q22+Q32+2.d0*sum(qtrans2*qtrans3))*Aq1))+&
         ...
         (tau_dot_tau_tab(t1,t2,t4,t5)*(mpi(impi)*(mpi2(impi) + 3.d0*Q12 + & 
         3.d0*Q22 + 4.d0*sum(qtrans1*qtrans2)) &
...
!!$    vn3lo_2PE = vn3lo_2PE + vn3lo2PE(q,p,r,t,s,u)
    ...
    vn3lo_2P1PE =  &
         ...
         *delta_tab(m1,m4)*delta_tab(t2,t5) &
         ...
         +tau1_dot_tauXtau_tab(t3,t1,t2,t6,t4,t5)* & 
         sigmaXsigma_dot_q_tab(m1,m2,m4,m5,qx1,qy1,qz1)*V3NF_F8(Q12,Aq1,impi))+&
         !132
         sigma_dot_q_ope252*&
         ...
         *delta_tab(m1,m4)*delta_tab(t3,t6) &
         ...
         +tau1_dot_tauXtau_tab(t2,t1,t3,t5,t4,t6)* & 
         sigmaXsigma_dot_q_tab(m1,m3,m4,m6,qx1,qy1,qz1)*V3NF_F8(Q12,Aq1,impi))+&
         !
         !321
         !
         sigma_dot_q_ope141*&
         ...
         *delta_tab(m3,m6)*delta_tab(t2,t5) &
         ...
         +tau1_dot_tauXtau_tab(t1,t3,t2,t4,t6,t5)* & 
         sigmaXsigma_dot_q_tab(m3,m2,m6,m5,qx3,qy3,qz3)*V3NF_F8(Q32,Aq3,impi))+&
         !
         !312
         !
         sigma_dot_q_ope252*&
         ...
         *delta_tab(m3,m6)*delta_tab(t1,t4) &
         ...
         +tau1_dot_tauXtau_tab(t2,t3,t1,t5,t6,t4)* & 
         sigmaXsigma_dot_q_tab(m3,m1,m6,m4,qx3,qy3,qz3)*V3NF_F8(Q32,Aq3,impi))+&
         !
         !213
         !
         sigma_dot_q_ope363*&
         ...
         *delta_tab(m2,m5)*delta_tab(t1,t4) &
         ...
         +tau1_dot_tauXtau_tab(t3,t2,t1,t6,t5,t4)* & 
         sigmaXsigma_dot_q_tab(m2,m1,m5,m4,qx2,qy2,qz2)*V3NF_F8(Q22,Aq2,impi))+&
         !
         !231
         !
         sigma_dot_q_ope141*&
         ...
         *delta_tab(m2,m5)*delta_tab(t3,t6) &
         ...
         !
         +tau1_dot_tauXtau_tab(t1,t2,t3,t4,t5,t6)* & 
         sigmaXsigma_dot_q_tab(m2,m3,m5,m6,qx2,qy2,qz2)*V3NF_F8(Q22,Aq2,impi))
...
!!$    cont_2PE = cont_2PE + cont2PE_N3LO(q,p,r,t,s,u)
    ...
    cont_2PE = &
         ...
         *delta_tab(t3,t6)*delta_tab(m1,m4) &
         ...
         -Q12*sigma_dot_sigma_tab(m1,m2,m4,m5))*Aq1*delta_tab(t3,t6)* & 
         delta_tab(t1,t4)*delta_tab(t2,t5)*delta_tab(m3,m6) )&
         !Eq 3.10
         ...
         *delta_tab(t2,t5)*delta_tab(m1,m4) &
         ...
         -Q12*sigma_dot_sigma_tab(m1,m3,m4,m6))*Aq1*delta_tab(t2,t5)* & 
         delta_tab(t1,t4)*delta_tab(t3,t6)*delta_tab(m2,m5) )&
         !Eq 3.10
         ...
         *delta_tab(t3,t6)*delta_tab(m2,m5) &
         ...
         -Q22*sigma_dot_sigma_tab(m1,m2,m4,m5))*Aq2*delta_tab(t3,t6)* &
         delta_tab(t2,t5)*delta_tab(t1,t4)*delta_tab(m3,m6) )&
         !Eq 3.10
         ...
         *delta_tab(t1,t4)*delta_tab(m2,m5) &
         ...
         -Q22*sigma_dot_sigma_tab(m3,m2,m6,m5))*Aq2*delta_tab(t1,t4)* &
         delta_tab(t2,t5)*delta_tab(t3,t6)*delta_tab(m1,m4) )&
         !Eq 3.10
         ...
         *delta_tab(t1,t4)*delta_tab(m3,m6) &
         ...
         -Q32*sigma_dot_sigma_tab(m3,m2,m6,m5))*Aq3*delta_tab(t1,t4)* &
         delta_tab(t3,t6)*delta_tab(t2,t5)*delta_tab(m1,m4) )&
         !Eq 3.10
         ...
         *delta_tab(t2,t5)*delta_tab(m3,m6) &
         ...
         -Q32*sigma_dot_sigma_tab(m3,m1,m6,m4))*Aq3*delta_tab(t2,t5)* &
         delta_tab(t3,t6)*delta_tab(t1,t4)*delta_tab(m2,m5) )&
         !Eq 3.10
    ...
    A_coeff = sigma_dot_q141*sigma_dot_q363*(&
         -gA2/(q12+mpi2(impi))*( (1-2*beta_8)*dot_product(qtrans1,qtrans3)**2*delta_tab(m2,m5) &
         - 2*imag*sigma2_dot_q1xq3*(&
         (1.-2.*beta_8)*dot_product(qtrans1,kmean2)+(1.+2.*beta_8)*dot_product(qtrans1,kmean1) ))&
    ...
    B_coeff = sigma_dot_q141*sigma_dot_q363*(&
         ...
         +2.*imag*dot_product(qtrans1,qtrans3)*( (1.-2.*beta_8)* &
         dot_product(qtrans1,kmean2) + (1.+2.*beta_8)*dot_product(qtrans1,kmean1) )*delta_tab(m2,m5))&
         !
         +2.*imag*dot_product(qtrans3,kmean3-kmean2)*delta_tab(m2,m5) + &
         2.*imag*gA2*(2.*beta_9-1.)*dot_product(qtrans1,kmean2)*delta_tab(m2,m5) &
         - sigma2_dot_q1xq3)&
         !
         +sigma_dot_q141*sigma3_dot_k3*(-2.*imag*gA2*(2*beta_9+1)*&
         dot_product(qtrans1,qtrans3))*delta_tab(m2,m5)

    relcorr_2PE = gA2/(32.*mnuc(imnuc)*fpi4)*1.d0/((q12+mpi2(impi))*(q32+mpi2(impi)))*(&
         tau_dot_tau_tab(t1,t3,t4,t6)*A_coeff*delta_tab(t2,t5) + &
         tau1_dot_tauXtau_tab(t3,t1,t2,t6,t4,t5)*B_coeff)
    
    !132
    A_coeff = sigma_dot_q141*sigma_dot_q252*(&
         -gA2/(q12+mpi2(impi))*( (1.-2.*beta_8)*dot_product(qtrans1,qtrans2)**2*delta_tab(m3,m6) - &
         2.*imag*sigma3_dot_q1xq2*(&
    ...
    B_coeff = sigma_dot_q141*sigma_dot_q252*(&
         ...
         +2.*imag*dot_product(qtrans1,qtrans2)*( (1.-2.*beta_8)*dot_product(qtrans1,kmean3) + &
         (1.+2.*beta_8)*dot_product(qtrans1,kmean1) )*delta_tab(m3,m6))&
         !
         +2.*imag*dot_product(qtrans2,kmean2-kmean3)*delta_tab(m3,m6) + &
         2.*imag*gA2*(2*beta_9-1)*dot_product(qtrans1,kmean3)*delta_tab(m3,m6) &
         - sigma3_dot_q1xq2)&
         !
         +sigma_dot_q141*sigma2_dot_k2*(-2.*imag*gA2*(2*beta_9+1)*&
         dot_product(qtrans1,qtrans2))*delta_tab(m3,m6)
    ...
    A_coeff = sigma_dot_q363*sigma_dot_q252*(&
         -gA2/(q32+mpi2(impi))*( (1.-2.*beta_8)*dot_product(qtrans3,qtrans2)**2*delta_tab(m1,m4) + &
         2.d0*imag*sigma1_dot_q2xq3*(&
    ...
    B_coeff = sigma_dot_q363*sigma_dot_q252*(&
         ...
         +2.*imag*dot_product(qtrans3,qtrans2)*( (1.-2.*beta_8)*dot_product(qtrans3,kmean1) + &
         (1.+2.*beta_8)*dot_product(qtrans3,kmean3) )*delta_tab(m1,m4))&
         !
         +2.*imag*dot_product(qtrans2,kmean2-kmean1)*delta_tab(m1,m4) + &
         2.*imag*gA2*(2*beta_9-1)*dot_product(qtrans3,kmean1)*delta_tab(m1,m4) &
         + sigma1_dot_q2xq3)&
         !
    ...
    A_coeff = sigma_dot_q363*sigma_dot_q141*(&
         -gA2/(q32+mpi2(impi))*( (1.-2.*beta_8)*dot_product(qtrans1,qtrans3)**2*delta_tab(m2,m5) + &
         2.*imag*sigma2_dot_q1xq3*(&
    ...
    B_coeff = sigma_dot_q363*sigma_dot_q141*(&
         ...
         +2.*imag*dot_product(qtrans1,qtrans3)*( (1.-2.*beta_8)*dot_product(qtrans3,kmean2) + &
         (1.+2.*beta_8)*dot_product(qtrans3,kmean3) )*delta_tab(m2,m5))&
         !
         +2.*imag*dot_product(qtrans1,kmean1-kmean2)*delta_tab(m2,m5) + &
         2.*imag*gA2*(2.*beta_9-1.)*dot_product(qtrans3,kmean2)*delta_tab(m2,m5) &
         + sigma2_dot_q1xq3)&
         !
    ...
    A_coeff = sigma_dot_q252*sigma_dot_q141*(&
         -gA2/(q22+mpi2(impi))*( (1.-2.*beta_8)*dot_product(qtrans1,qtrans2)**2*delta_tab(m3,m6) + &
         2.*imag*sigma3_dot_q1xq2*(&
    ...
    B_coeff = sigma_dot_q141*sigma_dot_q252*(&
         ...
         +2.*imag*dot_product(qtrans1,qtrans2)*( (1.-2.*beta_8)*dot_product(qtrans2,kmean3) + &
         (1.+2.*beta_8)*dot_product(qtrans2,kmean2) )*delta_tab(m3,m6))&
         !
         +2.*imag*dot_product(qtrans1,kmean1-kmean3)*delta_tab(m3,m6) + &
         2.*imag*gA2*(2*beta_9-1)*dot_product(qtrans2,kmean3)*delta_tab(m3,m6) &
         +sigma3_dot_q1xq2)&
         !
    ...
    relcorr_2PE = relcorr_2PE + gA2/(32.*mnuc(imnuc)*fpi4)*1.d0/((q22+mpi2(impi))*(q12+mpi2(impi)))*(&
         tau_dot_tau_tab(t1,t2,t4,t5)*A_coeff*delta_tab(t3,t6) + &
         tau1_dot_tauXtau_tab(t1,t2,t3,t4,t5,t6)*B_coeff)
    
    !213
    A_coeff = sigma_dot_q252*sigma_dot_q363*(&
         -gA2/(q22+mpi2(impi))*( (1-2*beta_8)*dot_product(qtrans3,qtrans2)**2*delta_tab(m1,m4) - &
         2*imag*sigma1_dot_q2xq3*(&
    ...
    B_coeff = sigma_dot_q363*sigma_dot_q252*(&
         ...
         +2.*imag*dot_product(qtrans3,qtrans2)*( (1.-2.*beta_8)*dot_product(qtrans2,kmean1) + &
         (1.+2.*beta_8)*dot_product(qtrans2,kmean2) )*delta_tab(m1,m4))&
         !
         +2*imag*dot_product(qtrans3,kmean3-kmean1)*delta_tab(m1,m4) + &
         2*imag*gA2*(2.*beta_9-1.)*dot_product(qtrans2,kmean1)*delta_tab(m1,m4) &
         - sigma1_dot_q2xq3)&
         !
...
!!$    relcorr_2PE = relcorr_2PE + rel2PE_3nf_corr(q,p,r,t,s,u)
    ...
    relcorr_1PE = 0d0
    !123
    f_coeff = ( (1.-2.*beta_8)*dot_product(qtrans1,qtrans3)*(cs_bar*sigma_dot_q251*delta_tab(m3,m6) + &
         CT_bar*sigma_dot_q361*delta_tab(m2,m5))&
         +2*imag*Ct_bar*sigmaXsigma_dot_q_tab(m2,m3,m5,m6,qx1,qy1,qz1)*( (1.-2.*beta_8)* &
         dot_product(qtrans1,kmean2)+(1.+2.*beta_8)*dot_product(qtrans1,kmean1)))/(q12+mpi2(impi))&
         +(2.*beta_9-1.)*(CS_bar*sigma_dot_q253*delta_tab(m3,m6) + CT_bar*sigma_dot_q363*delta_tab(m2,m5))&
         +2*imag*CT_bar*(2.*beta_9-1.)*0.5d0* &
         sigmaXsigma_dot_q_tab(m2,m3,m5,m6,nx5+nx2,ny5+ny2,nz5+nz2)!sigmaXsigma_dot_q(m2,m3,m5,m6,kmean2)
    
    g_coeff = -2*imag*CT_bar*(2.*beta_9+1.)*sigmaXsigma_dot_q_tab(m2,m3,m5,m6,qx1,qy1,qz1)
    
    relcorr_1PE = gA2/(8.0*mnuc(imnuc)*fpi2*(q12+mpi2(impi)))*&
         chp_tau_dot_tau(t1,t2,t4,t5)*(sigma_dot_q141*f_coeff &
         + sigma1_dot_k1*g_coeff)*delta_tab(t3,t6)
    
    !132
    f_coeff = ( (1.-2.*beta_8)*dot_product(qtrans1,qtrans2)*(CS_bar*sigma_dot_q361*delta_tab(m2,m5) + &
         CT_bar*sigma_dot_q251*delta_tab(m3,m6))&
         +2*imag*Ct_bar*sigmaXsigma_dot_q_tab(m3,m2,m6,m5,qx1,qy1,qz1)*( (1.-2.*beta_8)*&
         dot_product(qtrans1,kmean3)+(1.+2.*beta_8)*dot_product(qtrans1,kmean1)))/(q12+mpi2(impi))&
         +(2.*beta_9-1.)*(CS_bar*sigma_dot_q362*delta_tab(m2,m5) + CT_bar*sigma_dot_q252*delta_tab(m3,m6))&
         +2*imag*CT_bar*(2.*beta_9-1.)*&
         0.5d0*sigmaXsigma_dot_q_tab(m3,m2,m6,m5,nx6+nx3,ny6+ny3,nz6+nz3)!sigmaXsigma_dot_q(m3,m2,m6,m5,kmean3)
    
    g_coeff = -2*imag*CT_bar*(2.*beta_9+1.)*sigmaXsigma_dot_q_tab(m3,m2,m6,m5,qx1,qy1,qz1)
    
    relcorr_1PE = relcorr_1PE+ gA2/(8.0*mnuc(imnuc)*fpi2*(q12+mpi2(impi)))* &
         chp_tau_dot_tau(t1,t3,t4,t6)*(sigma_dot_q141*f_coeff &
         + sigma1_dot_k1*g_coeff)*delta_tab(t2,t5)
    
    !312
    f_coeff = ( (1.-2.*beta_8)*dot_product(qtrans3,qtrans2)*&
         (CS_bar*sigma_dot_q143*delta_tab(m2,m5) + CT_bar*sigma_dot_q253*delta_tab(m1,m4))&
         +2*imag*Ct_bar*sigmaXsigma_dot_q_tab(m1,m2,m4,m5,qx3,qy3,qz3)*( (1.-2.*beta_8)* &
         dot_product(qtrans3,kmean1)+(1.+2.*beta_8)*dot_product(qtrans3,kmean3)))/(q32+mpi2(impi))&
    ...
    relcorr_1PE = relcorr_1PE+ gA2/(8.0*mnuc(imnuc)*fpi2*(q32+mpi2(impi)))*&
         chp_tau_dot_tau(t3,t1,t6,t4)*(sigma_dot_q363*f_coeff &
         + sigma3_dot_k3*g_coeff)*delta_tab(t2,t5)
    
    !321
    f_coeff = ( (1.-2.*beta_8)*dot_product(qtrans3,qtrans1)*&
         (CS_bar*sigma_dot_q253*delta_tab(m1,m4) + CT_bar*sigma_dot_q143*delta_tab(m2,m5))&
         +2*imag*Ct_bar*sigmaXsigma_dot_q_tab(m2,m1,m5,m4,qx3,qy3,qz3)*( (1.-2.*beta_8)*&
         dot_product(qtrans3,kmean2)+(1.+2.*beta_8)*dot_product(qtrans3,kmean3)))/(q32+mpi2(impi))&
    ...
    relcorr_1PE = relcorr_1PE+ gA2/(8.0*mnuc(imnuc)*fpi2*(q32+mpi2(impi)))*&
         chp_tau_dot_tau(t3,t2,t6,t5)*(sigma_dot_q363*f_coeff &
         + sigma3_dot_k3*g_coeff)*delta_tab(t1,t4)

    !231
    f_coeff = ( (1.-2.*beta_8)*dot_product(qtrans2,qtrans1)*(CS_bar*sigma_dot_q362*delta_tab(m1,m4) + &
         CT_bar*sigma_dot_q142*delta_tab(m3,m6))&
         +2*imag*Ct_bar*sigmaXsigma_dot_q_tab(m3,m1,m6,m4,qx2,qy2,qz2)*( (1.-2.*beta_8)*&
         dot_product(qtrans2,kmean3)+(1.+2.*beta_8)*dot_product(qtrans2,kmean2)))/(q22+mpi2(impi))&
    ...
    relcorr_1PE = relcorr_1PE+ gA2/(8.0*mnuc(imnuc)*fpi2*(q22+mpi2(impi)))*&
         chp_tau_dot_tau(t2,t3,t5,t6)*(sigma_dot_q252*f_coeff &
         + sigma2_dot_k2*g_coeff)*delta_tab(t1,t4)

    !213
    f_coeff = ( (1.-2.*beta_8)*dot_product(qtrans2,qtrans3)*(CS_bar*sigma_dot_q142*delta_tab(m3,m6) + &
         CT_bar*sigma_dot_q362*delta_tab(m1,m4))&
         +2*imag*Ct_bar*sigmaXsigma_dot_q_tab(m1,m3,m4,m6,qx2,qy2,qz2)*( (1.d0-2.d0*beta_8)*&
         dot_product(qtrans2,kmean1)+(1.+2.*beta_8)*dot_product(qtrans2,kmean2)))/(q22+mpi2(impi))&
    ...
    relcorr_1PE = relcorr_1PE+ gA2/(8.0*mnuc(imnuc)*fpi2*(q22+mpi2(impi)))*&
         chp_tau_dot_tau(t2,t1,t5,t4)*(sigma_dot_q252*f_coeff &
...
!!$    END IF 
    ...
    vn3lo_ring = vn3lo_ring &
         ...
         *delta_tab(m3,m6)*delta_tab(t2,t5)*delta_tab(m1,m4)*delta_tab(m2,m5)&
         ...
         +sigma_dot_sigma_tab(m1,m3,m4,m6)*0.5*R10_tab(nx4-nx1,ny4-ny1,nz4-nz1,nx6-nx3,ny6-ny3,nz6-nz3))*&
         delta_tab(m2,m5)*delta_tab(t1,t4)*delta_tab(t2,t5)*delta_tab(t3,t6)&
         +chp_q_dot_qxsigma(qtrans1,qtrans3,m2,m5)*tau1_dot_tauXtau_tab(t1,t2,t3,t4,t5,t6)&
         *0.5*R11_tab(nx4-nx1,ny4-ny1,nz4-nz1,nx6-nx3,ny6-ny3,nz6-nz3)*delta_tab(m1,m4)*&
         delta_tab(m3,m6))*(gA/fpi)**6.0*hbarc
    ...
    vn3lo_ring = vn3lo_ring + &
         ...
         +chp_q_dot_qxsigma(qtrans1,qtrans3,m1,m4)*tau1_dot_tauXtau_tab(t1,t2,t3,t4,t5,t6)&
         *S7_tab(nx4-nx1,ny4-ny1,nz4-nz1,nx6-nx3,ny6-ny3,nz6-nz3)*delta_tab(m2,m5)*delta_tab(m3,m6) )*&
         (gA**4/fpi**6.0)*hbarc
...
!    call ring_functions(q12,q32,q22,z12,atanQ1,atanQ3,atanQ2,I4_q1q2,impi,R1,R2,R3,R4,R5,R6,R7,R8,R9,R10,R11,S1,S2,S3,S4,S5,S6,S7)
    vn3lo_ring = vn3lo_ring &
         ...
         *delta_tab(m2,m5)*delta_tab(t3,t6)*delta_tab(m1,m4)*delta_tab(m3,m6)&
         ...
         +sigma_dot_sigma_tab(m1,m2,m4,m5)*0.5*R10_tab(nx4-nx1,ny4-ny1,nz4-nz1,nx5-nx2,ny5-ny2,nz5-nz2))*&
         delta_tab(m3,m6)*delta_tab(t1,t4)*delta_tab(t3,t6)*delta_tab(t2,t5)&
         +chp_q_dot_qxsigma(qtrans1,qtrans2,m3,m6)*tau1_dot_tauXtau_tab(t1,t3,t2,t4,t6,t5)&
         *0.5*R11_tab(nx4-nx1,ny4-ny1,nz4-nz1,nx5-nx2,ny5-ny2,nz5-nz2)*delta_tab(m1,m4)*delta_tab(m2,m5))*&
         (gA/fpi)**6.0*hbarc
    ...
    vn3lo_ring = vn3lo_ring + &
         ...
         *S7_tab(nx4-nx1,ny4-ny1,nz4-nz1,nx5-nx2,ny5-ny2,nz5-nz2)*delta_tab(m3,m6)*delta_tab(m2,m5) )*&
         (gA**4/fpi**6.0)*hbarc
...
!    call ring_functions(q32,q12,q22,z23,atanQ3,atanQ1,atanQ2,I4_q3q2,impi,R1,R2,R3,R4,R5,R6,R7,R8,R9,R10,R11,S1,S2,S3,S4,S5,S6,S7)
    vn3lo_ring = vn3lo_ring &
         ...
         *delta_tab(m2,m5)*delta_tab(t1,t4)*delta_tab(m3,m6)*delta_tab(m1,m4)&
         ...
         +sigma_dot_sigma_tab(m3,m2,m6,m5)*0.5*R10_tab(nx6-nx3,ny6-ny3,nz6-nz3,nx5-nx2,ny5-ny2,nz5-nz2))*& 
         delta_tab(m1,m4)*delta_tab(t3,t6)*delta_tab(t1,t4)*delta_tab(t2,t5)&
         +chp_q_dot_qxsigma(qtrans3,qtrans2,m1,m4)*tau1_dot_tauXtau_tab(t3,t1,t2,t6,t4,t5)&
         *0.5*R11_tab(nx6-nx3,ny6-ny3,nz6-nz3,nx5-nx2,ny5-ny2,nz5-nz2)*delta_tab(m3,m6)*delta_tab(m2,m5))*& 
         (gA/fpi)**6.0*hbarc
    ...
    vn3lo_ring = vn3lo_ring + &
         ...
         *S7_tab(nx6-nx3,ny6-ny3,nz6-nz3,nx5-nx2,ny5-ny2,nz5-nz2)*delta_tab(m1,m4)*delta_tab(m2,m5) )*& 
         (gA**4/fpi**6.0)*hbarc
...
!!    call ring_functions(q32,q22,q12,z13,atanQ3,atanQ2,atanQ1,I4_q3q1,impi,R1,R2,R3,R4,R5,R6,R7,R8,R9,R10,R11,S1,S2,S3,S4,S5,S6,S7)
    vn3lo_ring = vn3lo_ring &
         ...
         *delta_tab(m1,m4)*delta_tab(t2,t5)*delta_tab(m3,m6)*delta_tab(m2,m5)&
         ...
         +sigma_dot_sigma_tab(m3,m1,m6,m4)*0.5*R10_tab(nx6-nx3,ny6-ny3,nz6-nz3,nx4-nx1,ny4-ny1,nz4-nz1))*& 
         delta_tab(m2,m5)*delta_tab(t3,t6)*delta_tab(t2,t5)*delta_tab(t1,t4)&
         +chp_q_dot_qxsigma(qtrans3,qtrans1,m2,m5)*tau1_dot_tauXtau_tab(t3,t2,t1,t6,t5,t4)&
         *0.5*R11_tab(nx6-nx3,ny6-ny3,nz6-nz3,nx4-nx1,ny4-ny1,nz4-nz1)*delta_tab(m3,m6)*delta_tab(m1,m4))*& 
         (gA/fpi)**6.0*hbarc
    vn3lo_ring = vn3lo_ring + &
         ...
         *S7_tab(nx6-nx3,ny6-ny3,nz6-nz3,nx4-nx1,ny4-ny1,nz4-nz1)*delta_tab(m2,m5)*delta_tab(m1,m4) )*& 
         (gA**4/fpi**6.0)*hbarc
...
!    call ring_functions(q22,q32,q12,z12,atanQ2,atanQ3,atanQ1,I4_q2q1,impi,R1,R2,R3,R4,R5,R6,R7,R8,R9,R10,R11,S1,S2,S3,S4,S5,S6,S7)
    vn3lo_ring = vn3lo_ring &
         ...
         *delta_tab(m1,m4)*delta_tab(t3,t6)*delta_tab(m2,m5)*delta_tab(m3,m6)&
         ...
         +sigma_dot_sigma_tab(m2,m1,m5,m4)*0.5*R10_tab(nx5-nx2,ny5-ny2,nz5-nz2,nx4-nx1,ny4-ny1,nz4-nz1))*& 
         delta_tab(m3,m6)*delta_tab(t2,t5)*delta_tab(t3,t6)*delta_tab(t1,t4)&
         +chp_q_dot_qxsigma(qtrans2,qtrans1,m3,m6)*tau1_dot_tauXtau_tab(t2,t3,t1,t5,t6,t4)&
         *0.5*R11_tab(nx5-nx2,ny5-ny2,nz5-nz2,nx4-nx1,ny4-ny1,nz4-nz1)*delta_tab(m2,m5)*delta_tab(m1,m4))*& 
         (gA/fpi)**6.0*hbarc
    vn3lo_ring = vn3lo_ring + &
         ...
         *S7_tab(nx5-nx2,ny5-ny2,nz5-nz2,nx4-nx1,ny4-ny1,nz4-nz1)*delta_tab(m3,m6)*delta_tab(m1,m4) )*& 
         (gA**4/fpi**6.0)*hbarc
   ...
   ! call ring_functions(q22,q12,q32,z23,atanQ2,atanQ1,atanQ3,I4_q2q3,impi,R1,R2,R3,R4,R5,R6,R7,R8,R9,R10,R11,S1,S2,S3,S4,S5,S6,S7)
    vn3lo_ring = vn3lo_ring &
         ...
         *delta_tab(m3,m6)*delta_tab(t1,t4)*delta_tab(m2,m5)*delta_tab(m1,m4)&
         ...
         +sigma_dot_sigma_tab(m2,m3,m5,m6)*0.5*R10_tab(nx5-nx2,ny5-ny2,nz5-nz2,nx6-nx3,ny6-ny3,nz6-nz3))*& 
         delta_tab(m1,m4)*delta_tab(t2,t5)*delta_tab(t1,t4)*delta_tab(t3,t6)&
         +chp_q_dot_qxsigma(qtrans2,qtrans3,m1,m4)*tau1_dot_tauXtau_tab(t2,t1,t3,t5,t4,t6)&
         *0.5*R11_tab(nx5-nx2,ny5-ny2,nz5-nz2,nx6-nx3,ny6-ny3,nz6-nz3)*delta_tab(m2,m5)*delta_tab(m3,m6))*& 
         (gA/fpi)**6.0*hbarc
    vn3lo_ring = vn3lo_ring + &
         ...
         *S7_tab(nx5-nx2,ny5-ny2,nz5-nz2,nx6-nx3,ny6-ny3,nz6-nz3)*delta_tab(m1,m4)*delta_tab(m3,m6) )*& 
         (gA**4/fpi**6.0)*hbarc
...
!!$    vn3lo_ring = vn3lo_ring + ring_3NF(q,p,r,t,s,u)
    ...
    chiral_3nf = freg_3nfnonlocal(k1,k2,k3)*freg_3nfnonlocal(k4,k5,k6)*hbarc**6*& 
         (v3nf_sum+vn3lo_2PE+cont_2PE+relcorr_1PE+relcorr_2PE+vn3lo_ring)/volume**2
  ...
  complex*16 function chiral_3nf2(kk1,m1,t1,kk2,m2,t2,kk3,m3,t3,kk4,m4,t4,kk5,m5,t5,kk6,m6,t6)
    ...
    complex*16 :: v3nf_sum1, v3nf_sum2, v3nf_sum3
    complex*16 :: sigma_dot_q252, sigma_dot_q142, sigma_dot_q251, sigma_dot_q141, & 
         sigma_dot_q_ope252, sigma_dot_q_ope141 
    complex*16 :: sigma_dot_q363, sigma_dot_q143, sigma_dot_q361, sigma_dot_q_ope363
    complex*16 :: sigma_dot_q253, sigma_dot_q362
    
    complex*16 :: sigma3_dot_q1xq2,sigma2_dot_q1xq3,sigma1_dot_q2xq3
    complex*16 :: sigma3_dot_k3,sigma2_dot_k2,sigma1_dot_k1
